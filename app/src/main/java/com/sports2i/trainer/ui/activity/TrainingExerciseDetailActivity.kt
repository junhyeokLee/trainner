...
import com.sports2i.trainer.databinding.ActivityTrainingExerciseDetailBinding
...
@AndroidEntryPoint
class TrainingExerciseDetailActivity : BaseActivity<ActivityTrainingExerciseDetailBinding>({ActivityTrainingExerciseDetailBinding.inflate(it)}),
    OnMapReadyCallback {
...
//  네트워크 상태 관찰자를 합쳐서 로딩 상태를 처리
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        clickBack()
        this@TrainingExerciseDetailActivity.onBackPressedDispatcher.addCallback(this, onBackPressedCallback)
    }
    private fun init() {
        // 각종 뷰 및 UI 초기화 작업
        ...
        myId = Preferences.string(Preferences.KEY_USER_ID)
    ...
    private fun setUserSpinner(){
        ...
        customUserSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener{
            ...
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                bottomSheetFragment = CommentBottomSheet(this@TrainingExerciseDetailActivity, selectedUserId!!, selectedDateTime!!, trainingViewModel)
        ...
        binding.tvRecord.setOnClickListener {
            ...
            val videoDataRecordClickListener = DialogInterface.OnClickListener { dialog, _ ->
                ...
                when(exerciseId) {
                    ...
                    "E04" -> { // 벤치프레스 이지만 테스트로 데드리프트 카운팅 영상으로
                        getPoseCamera()
                        dialog.dismiss()
                    }
    ...
    private fun setupRecyclerView() {
        ...
        if(!checkAdminAuthority() && selectedUserId.equals(myId)){ // 관리자 권한이 아닐때 또는 선택한 아이디가 내아이디일때 (수정가능)
                ...
                override fun onItemClick(position: Int, trainingExerciseList: List<TrainingInfoResponse>) {
                    ...
                    dialogFragment.mListener = object : CustomExerciseUpdateDialogFragment.OnSaveClickListener {
                        override fun onSaveClick(trainingExerciseList: List<TrainingInfoResponse>) {
                            ...
                            Log.e("트레이닝인포Adapter리스트",modifiedTrainingInfoList.toString())
        ...
        when(trainingSub.url) {
            "" -> {
                ...
                if(trainingSub.startDate.isEmpty() && trainingSub.endDate.isEmpty()) {
                    ...
                    if(checkAdminAuthority() || !selectedUserId.equals(myId)){ // 관리자 권한일때 또는 선택한 아이디가 내아이디가 아닐경우
                        binding.tvRecordDesc.visibility = View.GONE
                        binding.tvRecord.visibility = View.GONE
                    } else {
                        binding.tvRecordDesc.visibility = View.VISIBLE
                    ...
                    if(checkAdminAuthority() || !selectedUserId.equals(myId)){ // 관리자 권한일때
                        binding.layoutEditRecord.visibility = View.GONE
                        binding.tvRecordDesc.visibility = View.GONE
                        binding.tvRecord.visibility = View.GONE
                    } else {
                    ...
                    if (trackingData != null) {
                        ...
                        if (trackingData.pathPoints.isNotEmpty()) {
                            ...
                            val startBitmapDescriptor = BitmapDescriptorFactory.fromBitmap(
                                FileUtil.resizeBitmap(
                                    FileUtil.getBitmapFromVector(this, R.drawable.ic_circle_hollow), 50, 50))
                            ...
                            val endBitmapDescriptor = BitmapDescriptorFactory.fromBitmap(
                                FileUtil.resizeBitmap(
                                    FileUtil.getBitmapFromVector(this, R.drawable.ic_tracking_finish), 50, 50))
...
//                            googleMap?.moveCamera(cameraUpdate)
                            ...
                            googleMap?.setOnMapClickListener {
                                val intent = Intent(this@TrainingExerciseDetailActivity, TrackingMapActivity::class.java)
                                intent.putExtra("exerciseId", exerciseId)
                                intent.putExtra("userId",selectedUserId)
                                intent.putExtra("trainingTime",trainingTime)
                                intent.putExtra("selectedDate",selectedDateTime)
        ...
        when(trainingSub.rpe){
            0 -> {
                if(checkAdminAuthority() || !selectedUserId.equals(myId)){ // 관리자 권한일때 또는 선택한 아이디가 내아이디가 아닐때
                    binding.btnRpe.visibility = View.GONE
                    binding.tvRpe.text = "운동 자각도: 0"
                    binding.tvTss.text = "훈련부하: 0"
                } else{
...
//        exerciseDetailList = groupedExerciseList as MutableList<TrainingInfoResponse>
    ...
    private fun observeTrainingSubSearch() = observeNetworkState(
        trainingViewModel.trainingSubSearchState, { handlerTrainingSubDetailSuccess(it.data.data)},
        {
            ...
            checkEmptyData(trainingSub)
        Log.d("trainingSubSearch error : ", it.message)
        })
    ...
    private fun handlerGroupInfoSuccess(groupInfos: MutableList<GroupInfo>) {
        ...
        val itemBinder = object : ItemBinder<GroupInfo> {
            override fun bindItem(view: View, item: GroupInfo, isDropDown: Boolean) {
                ...
                if (isDropDown) {
                    textView.setTextColor(this@TrainingExerciseDetailActivity.resources.getColor(android.R.color.black))
                } else {
                    textView.setTextColor(this@TrainingExerciseDetailActivity.resources.getColor(android.R.color.white))
                }
            }
        }
    ...
    private fun handlerTrainingDetailExerciseSuccess(trainingDetailExercise: List<TrainingInfoResponse>) {
        Log.e("trainingDetailExercise", trainingDetailExercise.toString())
...
//          val sleepTime by healthConnectViewModel.flowSleepTime.collectAsState()
    ...
    private fun showStartTimeEmptyDialog(){
        ...
        val alertDialog = AlertDialog.Builder(this@TrainingExerciseDetailActivity)
            .setMessage(message)
