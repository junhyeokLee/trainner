...
class ExerciseTimeTabAdapter(fragment: Fragment) : FragmentStateAdapter(fragment) {
    private val fragments = ArrayList<ExerciseTimeFragment>()
    private val tabTitles = ArrayList<String>() // 탭에 표시될 타이틀들을 저장할 리스트

    private var userId: String = ""
    private var dateTime: String = ""

    fun addFragment(fragment: ExerciseTimeFragment, title: String) {
        fragments.add(fragment)
        tabTitles.add(title) // 탭 타이틀을 추가
    }

    fun updateFragment(position: Int, fragment: ExerciseTimeFragment) {
    ...
    fun getFragment(position: Int): ExerciseTimeFragment? {
