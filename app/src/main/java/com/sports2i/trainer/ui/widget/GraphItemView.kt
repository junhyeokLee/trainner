...
import com.sports2i.trainer.data.model.TrainingOverallSample

@SuppressLint("ViewConstructor", "SetTextI18n")
class GraphItemView constructor(context: Context, sampleType: Int, list: MutableList<TrainingOverallSample>): RelativeLayout(context) {
...
//    }
    ...
    private fun line(num: Int, num2: Int) {
        ...
        params.bottomMargin = ((num + 22 + num2) * resources.displayMetrics.density).toInt()
        params.addRule(ALIGN_PARENT_BOTTOM)
        ...
        params.bottomMargin = ((num + 28 + num2) * resources.displayMetrics.density).toInt()
    ...
    private fun graph(list: ArrayList<Int>, type: Int): LinearLayoutCompat {
        ...
        for (i in 1 until list.size) {
            ...
            view.setBackgroundColor(ContextCompat.getColor(context, color(type + 10)))
            container.addView(view)
            layout.addView(container)
            view = View(context)
    ...
    private fun graph(num: Int, data: String, type: Boolean, layoutParams: LinearLayoutCompat.LayoutParams): RelativeLayout {
        val layout = RelativeLayout(context)
        layout.setBackgroundColor(ContextCompat.getColor(context, R.color.transparent))
        layout.layoutParams = layoutParams
        ...
        layout.addView(textView)
        val view = View(context)
        params = LayoutParams((10 * resources.displayMetrics.density).toInt(), (num * resources.displayMetrics.density).toInt())
        params.bottomMargin = (6 * resources.displayMetrics.density).toInt()
        params.addRule(ABOVE, 1)
        params.addRule(CENTER_IN_PARENT)
        view.id = 2
        ...
        if (type) {
            ...
            textView.background.colorFilter = BlendModeColorFilter(ContextCompat.getColor(context, R.color.green), BlendMode.SRC_ATOP)
    ...
    init {
        this.layoutParams = LinearLayoutCompat.LayoutParams(LinearLayoutCompat.LayoutParams.MATCH_PARENT, ((when {sampleType != 1 -> 150 else -> 180 }) * resources.displayMetrics.density).toInt())
        ...
        if (sampleType < 3) {
            ...
            layout.layoutParams = LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.MATCH_PARENT)
            layout.orientation = LinearLayoutCompat.HORIZONTAL
            val params = LinearLayoutCompat.LayoutParams((76 * resources.displayMetrics.density.toInt()), LinearLayoutCompat.LayoutParams.MATCH_PARENT)
            when (sampleType) {
                1 -> {
                    var sum = 0
                    for (i in 0 until list.size) {
                        sum += list[i].injuryIndex.toInt()
                    }
                    /** 선수
                     ...
                     * margin = 0
                     * */
                    val isTrainingDate = false
                    val margin = 30
                    line(sum / list.size, margin)
                    for (i in 0 until 31) {
                        layout.addView(graph(list[i].injuryIndex.toInt(), list[i].userName, isTrainingDate, params))
                    }
                }
                2 -> {
                    for (i in 0 until 31) {
                        val trainingDate = list[i].trainingDate
                        layout.addView(graph(list[i].sleepIndex.toInt(), list[i].sleepDuration.toInt(), "${trainingDate.substring(trainingDate.length - 2, trainingDate.length)}일", params))
                    }
                }
            }
            ...
            when (sampleType) {
                3 -> {
                    ...
                    for (i in 0 until list.size) {
                        sampleList.add(list[i].injuryIndex.toInt())
                    }
                    ...
                    textView.textSize = 11F
                    textView.setTextColor(ContextCompat.getColor(context, R.color.black))
                    textView.text = "${list[0].injuryIndex.toInt()}%"
                    layout.addView(textView)
                    textView = AppCompatTextView(context)
                    params = LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT)
                    params.leftMargin = (14 * resources.displayMetrics.density).toInt()
                    ...
                    for (i in 1 until list.size) {
                        ...
                        textView.textSize = 11F
                        textView.setTextColor(ContextCompat.getColor(context, R.color.black))
                        textView.text = "${list[i].injuryIndex.toInt()}%"
                        layout.addView(textView)
                        textView = AppCompatTextView(context)
                        params = LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT)
                        params.leftMargin = ((14 + (i * 60)) * resources.displayMetrics.density).toInt()
